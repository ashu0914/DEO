...
def clean_action_command(cmd, structural_words):
    """Song names ya search terms se words ko remove karne ka advanced logic, 
    taaki 'Dil Se' jise gaano me se 'se' delete na ho jaye."""
    words = cmd.split()
    cleaned_words = [w for w in words if w not in structural_words]
...
def execute(command, ai_reply=""):
    ...
    if any(x in cmd_lower for x in ["youtube", "یوٹیوب", "यूट्यूब"]):
        stop_words = ["youtube", "pe", "par", "chalao", "play", "search", "kholo", "open", "यूट्यूब", "یوٹیوب"]
        search_query = clean_action_command(cmd_lower, stop_words)
    ...
    # ---------------- 2. SMART SPOTIFY & MUSIC ENGINE ----------------
        ...
        stop_words = ["spotify", "स्पॉटिफाई", "se", "song", "gaana", "chalao", "play", "music", "chalana", "gana"]
        song_name = clean_action_command(cmd_lower, stop_words)
    ...
    # ---------------- 6. WEB GOOGLE SEARCHES ----------------
        ...
        stop_words = ["search", "google par dhoondo", "google pe", "what is", "dhoondo", "nova"]
        search_term = clean_action_command(cmd_lower, stop_words)
    ...
    # ---------------- 8. KILL PROCESSES (Close Active Apps) ----------------
        ...
        stop_words = ["close", "band", "karo", "app", "kill"]
        app_to_close = clean_action_command(cmd_lower, stop_words)
